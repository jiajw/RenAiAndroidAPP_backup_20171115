...
public class PhoneUtil {
	...
	public PhoneUtil(String mobile) {
	...
	public void setMobile(String mobile) {
		...
		/** */
		/** 第一步判断中国移动 */
		if (mobile.matches(PhoneUtil.regMobileStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(0);
			this.setLawful(true);
			if (mobile.matches(PhoneUtil.regMobile3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第二步判断中国联通 */
		else if (mobile.matches(PhoneUtil.regUnicomStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(1);
			this.setLawful(true);
			if (mobile.matches(PhoneUtil.regUnicom3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第三步判断中国电脑 */
		else if (mobile.matches(PhoneUtil.regTelecomStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(2);
			this.setLawful(true);
			if (mobile.matches(PhoneUtil.regTelocom3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第四步判断座机 */
		if (mobile.matches(PhoneUtil.regPhoneString)) {
			this.mobile = mobile;
			this.setFacilitatorType(0);
			this.setLawful(true);
			if (mobile.matches(PhoneUtil.regMobile3GStr)) {
	...
	public static boolean checkPhoneNumber(String number) {
		if (number.length() == 11
				&& (number.startsWith("13") || number.startsWith("14")
						|| number.startsWith("15") || number.startsWith("16")
