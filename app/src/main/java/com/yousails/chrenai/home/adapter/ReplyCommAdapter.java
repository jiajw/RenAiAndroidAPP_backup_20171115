...
public class ReplyCommAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    //加载更多item 状态切换
    ...
    public ReplyCommAdapter(Context context,String publisherId, List<CommentBean> commentBeanList){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_NORMAL) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.reply_comment_item, parent, false);
...
//            ((CommentHolder) holder).createView.setText(TimeUtil.getDistanceTime(commentBean.getCreated_at()));
            ...
            if(0==position){
                ((CommentHolder) holder).replyView.setVisibility(View.GONE);
                ((CommentHolder) holder).bottomLayout.setVisibility(View.VISIBLE);
                if( getItemCount()>1){
                    ((CommentHolder) holder).bottomCommView.setVisibility(View.VISIBLE);
                }else{
                    ((CommentHolder) holder).bottomCommView.setVisibility(View.GONE);
                }

                ((CommentHolder) holder).deleteLayout.setVisibility(View.GONE);

            }else{
                ((CommentHolder) holder).replyView.setVisibility(View.VISIBLE);
                ((CommentHolder) holder).bottomLayout.setVisibility(View.GONE);
    ...
    class CommentHolder extends RecyclerView.ViewHolder {
        ...
        private TextView bottomCommView;
        ...
        public CommentHolder(View itemView) {
            ...
            bottomCommView=(TextView) itemView.findViewById(R.id.tv_bottom_comment);
